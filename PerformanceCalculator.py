...
def main(seed):
   ...
   # dataset = pd.read_csv( 'inputFieldsAtLeast1+MethodCalls.txt', sep= ',', index_col=False) 
    ...
    print('Test Set: Chess + Training set: Gantt, iTrust, JHotDraw')
    X_test, y_test, X_train, y_train =GetResults2(dataset,0,1,2,3)
    ComputePrecisionRecall(X_train, X_test, y_train, y_test)
    
    '''print('Test Set: Gantt + Training set: Chess, iTrust, JHotDraw ')
    X_test, y_test, X_train, y_train =GetResults2(dataset,1,0,2,3)
    ComputePrecisionRecall(X_train, X_test, y_train, y_test)
    
    print('Test set: iTrust + training Set: Chess, Gantt, JHotDraw')
    X_test, y_test, X_train, y_train =GetResults2(dataset,2,0,1,3)
    ComputePrecisionRecall(X_train, X_test, y_train, y_test)
    
    print('Test set: JHotDraw + training Set: Chess, Gantt, iTrust')
    X_test, y_test, X_train, y_train =GetResults2(dataset,3,0,1,2)
    ComputePrecisionRecall(X_train, X_test, y_train, y_test)'''
    
    


def GetResults2(dataset,test,train1, train2, train3):   
    X_train={}
    X_test={}
    y_train={}
    y_test={}
    ...
    TrainingSet=pd.concat([trainSet1, trainSet2, trainSet3])
...
def ComputePrecisionRecall(X_train, X_test, y_train, y_test):
    ...
    while i<len(probs_list):
            print('i ',i, ' TOTAL ',len(probs_list))
            flag=False
